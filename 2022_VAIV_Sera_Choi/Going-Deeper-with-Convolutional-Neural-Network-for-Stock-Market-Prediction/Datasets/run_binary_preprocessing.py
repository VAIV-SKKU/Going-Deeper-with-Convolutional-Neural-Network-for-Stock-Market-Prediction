...
try:
    if onlytraining:
        ...
        subprocess.call(
            f'python preproccess_binclass.py -m img2dt -i dataset2/{windows_length}_{dimension}/{symbol}/training -lf labels/{symbol}_training_label_{windows_length}.txt', shell=True)
        print('{GREEN}Labelling Training Data Done\n{END}'.format(**formatters))
    if onlytesting:
        # labelling data testing
        print('{RED}\nLabelling Testing Data{END}'.format(**formatters))
        subprocess.call(
            f'python preproccess_binclass.py -m img2dt -i dataset2/{windows_length}_{dimension}/{symbol}/testing -lf labels/{symbol}_testing_label_{windows_length}.txt', shell=True)
