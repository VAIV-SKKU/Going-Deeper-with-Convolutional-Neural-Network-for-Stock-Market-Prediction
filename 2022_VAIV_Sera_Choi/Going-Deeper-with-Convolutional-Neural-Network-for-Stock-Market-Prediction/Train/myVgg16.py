...
def build_model(SHAPE, nb_classes, bn_axis, weights_path=None):
    # We can't use ResNet50 directly, as it might cause a negative dimension
    # error.
    print("{ method - build_model }")
    seed = None
    if seed:
        np.random.seed(seed)
...
def main():
    ...
    seq_len = '20'#args.input.split('_')[-2]    #dataset 이름 바꾸기
    batch_ = args.batch_size
    ...
    path_checkpoint = 'checkpoints4/VGG16_' + f'{batch_}_{seq_len}' + '_' + str(args.dimension) + '_' + period_name[1] + "/model-{epoch:04d}.h5"
    path_dir_checkpoint = os.path.dirname(path_checkpoint)
    # Create a callback that saves the model's structure by five epochs
    callback_cp = tf.keras.callbacks.ModelCheckpoint(filepath=path_checkpoint, period=10)
    callback_stop = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=5, restore_best_weights=True, mode='min', verbose=1)
    callback_csv = tf.keras.callbacks.CSVLogger(f'training_log4/VGG16_{batch_}_{seq_len}_{args.dimension}_{period_name[1]}.csv')
    # Fit the model
    history = model.fit(X_train, Y_train, batch_size=batch_size, epochs=epochs, validation_data=(X_test, Y_test), callbacks=[callback_cp, callback_csv, callback_stop])

    # Save Model or creates a HDF5 file
    model.save('saved_model4/{}epochs_{}batch_vgg16_model_{}.h5'.format(
        epochs, batch_size, period_name[1]), overwrite=True)
    ...
    plt.savefig('pretrained_models/pretrained_{}epochs_{}batch_pretrained_models_vgg16_{}\n'.format(
    ...
    f_output.write('=======\n')
    f_output.write('{}epochs_{}batch_vgg16_{}_0701\n'.format(
        epochs, batch_size, period_name[1]))
    f_output.write('TN: {}\n'.format(tn))
    f_output.write('FN: {}\n'.format(fn))
    f_output.write('TP: {}\n'.format(tp))
