...
def main():
    ...
    model = keras.models.load_model('saved_model2/50epochs_128batch_resnet50_model_dataset_Taiwan50.h5')
    adam=keras.optimizers.Adam(learning_rate=1.0e-4)
    model.compile(optimizer=adam,
                  loss='categorical_crossentropy', metrics=['accuracy'])
    path_checkpoint = 'checkpoints2/ResNet50_' + f'{batch_size}_{seq_len}' + '_' + str(args.dimension) + '_' + period_name[1] +"/model-after50epochs-{epoch:04d}.h5"
    ...
    callback_csv = tf.keras.callbacks.CSVLogger(f'training_log2/ResNet50_{batch_size}_{seq_len}_{args.dimension}_{period_name[1]}_after50epochs.csv')
    # Fit the model
    model.fit(X_train, Y_train, batch_size=batch_size, epochs=epochs, callbacks=[callback_cp, callback_csv, callback_stop])

    # Save Model or creates a HDF5 file
    model.save('saved_model2/{}epochs_{}batch_resnet50_model_{}.h5'.format(
        epochs + 50, batch_size, data_directory.replace("/", "_")), overwrite=True)
